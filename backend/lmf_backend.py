...
class LMFBackend:
    """OpenAI-compatible adapter for the existing LLaMA-Factory API."""
    ...
    def generate(self, request: InferenceRequest) -> InferenceResponse:
        ...
        try:
            ...
            return self._error_response(
                started_at,
                "timeout",
                f"Model API request timed out after {elapsed:.1f}s (client timeout={self.timeout_seconds}s).",
                retryable=True,
            )
            ...
            return self._error_response(
                started_at,
                f"http_{exc.code}",
                f"Model API HTTP {exc.code}: {body[:1000]}",
            ...
            return self._error_response(
                started_at,
                "invalid_response",
                f"Model API returned an invalid response: {exc}",
            )
        except OSError as exc:
            return self._error_response(
                started_at,
                "transport_error",
                f"Model API transport failed: {exc}",
                retryable=True,
            )
