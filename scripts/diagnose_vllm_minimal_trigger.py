...
def build_variants(tokenizer: Any, messages: list[dict[str, str]], canonical: str) -> dict[str, str]:
    ...
    short_system = "Return one JSON object only. Do not explain."
    simple_user = "Read README section 1 and extract: goals, risks, architecture."
    long_user = user_text
    long_system = system

    variants: dict[str, str] = {
        "simple_plain": f"{short_system}\n\n{simple_user}",
        "simple_deepseek_roles": f"{short_system}{user}{simple_user}{assistant}{think}",
        "simple_deepseek_roles_with_bos": f"{bos}{short_system}{user}{simple_user}{assistant}{think}",
        "long_system_plain": f"{long_system}\n\n{simple_user}",
        "long_system_deepseek_roles_simple_user": f"{long_system}{user}{simple_user}{assistant}{think}",
        "long_system_deepseek_roles_long_user": f"{long_system}{user}{long_user}{assistant}{think}",
        "short_system_deepseek_roles_long_user": f"{short_system}{user}{long_user}{assistant}{think}",
        "canonical": canonical,
        "canonical_special_user_only": canonical.replace("<|user|>\n", user),
        "canonical_special_user_plus_assistant": canonical.replace("<|user|>\n", user) + assistant + think,
        "canonical_assistant_suffix": canonical + "\n" + assistant + think,
    }

    # Vary exact system length while keeping DeepSeek role tokens.
    system_lines = [line for line in long_system.splitlines() if line.strip()]
    for count in (1, 2, 4, 8, 12):
        prefix = "\n".join(system_lines[:count])
        variants[f"system_first_{count}_lines_deepseek_roles"] = f"{prefix}{user}{simple_user}{assistant}{think}"

    return variants


def main() -> int:
    parser = argparse.ArgumentParser(description="Find minimal prompt structure that triggers vLLM bang first token.")
    ...
    parser.add_argument("--output", type=Path, default=Path("data/diagnostics/vllm_minimal_trigger.json"))
    ...
    variants = build_variants(tokenizer, messages, canonical)
    ...
    for item in results:
        bucket = summary["by_variant"].setdefault(str(item["variant"]), {"total": 0, "ok": 0, "bang_first": 0})
        bucket["total"] += 1
        if item["ok"]:
            bucket["ok"] += 1
    ...
    report = {
        "diagnostic": "vllm_minimal_trigger",
