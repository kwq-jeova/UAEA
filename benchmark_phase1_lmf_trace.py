...
class LMFTraceBenchmarkRunner(phase1_benchmark.BenchmarkRunner):
    def __init__(
        self,
        project_root: Path,
        ...
        super().__init__(project_root, mode="real-lmf-trace")
            ...
            trace_defaults.update(
                {
                    ...
                    "benchmark_level": active_case.level,
                    "benchmark_description": active_case.description,
                }
            )
        backend = create_backend(self.backend_settings)
        traced_backend = TracingBackend(backend, self.trace_sink, trace_defaults)
        model = BackendModelClient(traced_backend)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run UAEA Phase-1 Runtime benchmark on LMFBackend with trace JSONL.")
    parser.add_argument("--case", default="", help="Run a single case_id, e.g. L0-01.")
    parser.add_argument("--level", default="", help="Run a benchmark level such as L0, L1, L2, or all.")
    parser.add_argument("--base-url", default=DEFAULT_BACKEND_URLS["lmf"])
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Append inference trace records to this JSONL file.",
    )
    ...
    trace_sink = JsonlTraceSink(args.trace_output)
    trace_defaults = {
        "backend": "lmf",
    ...
    runner = LMFTraceBenchmarkRunner(PROJECT_ROOT, backend_settings, trace_sink, trace_defaults)
    results = runner.run(phase1_benchmark.select_cases(args.case, args.level))
    for result in results:
        phase1_benchmark.print_result(result)

    payload = {
        "benchmark": "phase1_runtime_lmf_trace_benchmark",
        "mode": runner.mode,
        "backend": {
    ...
    if args.json or args.output is not None:
        ...
        if args.output is not None:
            ...
            default_path = runner.results_root / f"phase1_runtime_lmf_trace_benchmark_{phase1_benchmark.utc_stamp()}.json"
